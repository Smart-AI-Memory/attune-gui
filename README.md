...
| **Staleness + regenerate panel** | A status row above the frontmatter form shows whether the open template still matches its source — *fresh* / *stale* / *not generated* / *sources-missing* — via `GET /api/corpus/<id>/template/provenance`, which recomputes the source hash with `attune_author.staleness` (pure Python, no LLM). When it's stale and bound to a feature, a **Regenerate from source** button starts a `template.regenerate` job (`POST /api/corpus/<id>/template/regenerate`, token-guarded) that re-renders via `attune-author`; the on-disk rewrite flows back through the existing `file_changed` WS reload. Live regen needs Claude Code subscription auth (same as living-docs regen); tests mock the generator. |
